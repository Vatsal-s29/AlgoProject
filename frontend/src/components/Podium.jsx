...
const Podium = () => {
    ...
    const calculateSuccessRate = (user) => {
        return user.total > 0 ? (user.accepted / user.total) * 100 : 0;
    };

    // Multi-criteria sorting function
    const sortUsers = (users) => {
        return [...users].sort((a, b) => {
            // 1. Compare by rating (highest first)
            ...
            // 2. Compare by number of problems solved (highest first)
            const problemsA = a.uniqueProblemsSolved || 0;
            const problemsB = b.uniqueProblemsSolved || 0;
            if (problemsA !== problemsB) {
                return problemsB - problemsA;
            }

            // 3. Compare by success rate (highest first)
            ...
            // 4. Compare by name (lexicographic/alphabetical order)
    ...
    // Sort users based on multiple criteria
    const sortedUsers = sortUsers(topThree);
        ...
        <div className="mb-12">
            <div className="flex justify-center items-end space-x-8 mb-8">
                {sortedUsers
                    ...
                    .sort(
                        (a, b) =>
                            getPodiumOrder(a.rank) - getPodiumOrder(b.rank)
                    )
                    .map(({ user, rank }) => {
                        const rating = calculateRating(user);
                        const successRate =
                            calculateSuccessRate(user).toFixed(1);
                        ...
                        return (
                            ...
                            >
                                ...
                                {/* Podium Base */}
                                <div
                                    className={`w-32 ${getPodiumHeight(rank)} ${
                                        rank === 1
